...
let booksPerPage = 5;
// Show current results per page
let currentTotalResults = 1;
...
function updatePaginationInfo() {
    ...
    // Calculate by showing the current show result by all books as per next page 
    booksPerPage = parseInt(document.getElementById("showEntries").value);
    const startIndexShowResult = (currentPage - 1) * booksPerPage + 1;
    const endIndexShowResult = Math.min(startIndexShowResult + booksPerPage - 1, totalResults);

    // Page 1 of (totalpages)
    paginationInfo.textContent = `Page ${currentPage} of ${totalPages}`;
    
    // Showing 1 - 5 of 20 results
    paginationShowResult.textContent = `Showing ${endIndexShowResult} of ${totalResults} results`;
}
